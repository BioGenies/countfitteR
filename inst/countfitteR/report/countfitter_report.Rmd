...
Models selected: `r paste0(nice_model_names[input[["chosen_models"]]], collapse = ", ")`.

```{r}
if(input[["mean_value_rep"]]) {
  cat("\n",readLines("../readmes/mean_value/1.md"), sep = "    \n")
  plot(plot_fitlist(fits()))
...
if(input[["decision_rep"]]) {
  ...
  cat(decide(summarized_fits(), input[["sep_exp"]]))
}
```
...
if(input[["cmp_distr_rep"]]) {
  ...
  plot(plot_fitcmp(compared_fits()))
  cat("\n")
